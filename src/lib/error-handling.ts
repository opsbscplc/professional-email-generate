...
export class AppError extends Error {
  ...
  private getUserFriendlyMessage(message: string, code?: string): string {
    ...
    // Enhanced fallback message with debugging guidance
    if (process.env.NODE_ENV === 'development') {
      return `Error: ${message}. Check browser console (F12) for details.`
    }

    return 'Something went wrong. Please check the console (F12) for more details and try again.'
  }
}
